...
// 创建一个空数组 I 用于存储项目集,向 I 中添加 G 的初始项目
// 创建一个空数组 C 用于存储项目集规范族,将初始项目的闭包添加到项目集规范族 C 中。
// 创建一个数组 V_and_T，包含了文法中的所有终结符和非终结符。
// 创建一个队列数组 E，用于记录尚未遍历的项目集,将项目集规范族的初始项目集 C[0] 添加到队列 E 中。
// 进入一个循环，不断处理队列 E 中的项目集，直到队列为空, 在每次迭代中，从队列 E 中取出第一个项目集 IT，准备对其进行处理。
// 针对项目集 IT，通过终结符和非终结符遍历，计算所有可能的后继项目集 J，并通过转移函数 GO 获取后继项目集 J。
// 判断后继项目集是否存在：如果存在，则检查后继项目集 J是否已经在项目集规范族 C 中：
// 如果不在，将其添加到 C 中，并同时添加到队列 E 中以便后续处理。
// 继续下一个项目集：将当前项目集 IT 从队列 E 中移除，以便处理下一个项目集。
...
function SLR1Analysis (G, ACTION, GOTO, token) {
  ...
  while (true) {
    ...
    //将使用第 k 个产生式进行规约，将右部符号依次弹出，然后将左部非终结符压入符号栈。
    // 根据规约的产生式长度 n，从状态栈和符号栈中弹出相应数量的状态和符号。
    // 根据状态栈栈顶状态和规约的非终结符，在 GOTO 表中查询，并将新的状态压入状态栈。
    // 在记录 write 中添加提示信息，表示查询 GOTO 表并将新状态压入状态栈。
    else if (string[0] == 'r') {
